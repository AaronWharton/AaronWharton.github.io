...
结果证明，实参传递给函数的只是一个 slice 的副本，它们不是指向同一个内存地址的。在 `main` 函数和 `modify2` 函数里面我们打印了 s[0] 的内存地址，发现它们的内存地址是相同的，所以当我们在 `modify2` 函数里面修改 s[0] 会影响 s[0] 的原始值，从而解决了我们的疑惑。


### Everything in Go is passed by value.

事实证明： Go 中所有数据都是通过值传递的。
